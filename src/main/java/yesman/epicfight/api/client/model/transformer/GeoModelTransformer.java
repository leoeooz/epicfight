...
public class GeoModelTransformer extends HumanoidModelTransformer {
	...
	@OnlyIn(Dist.CLIENT)
	public record GeoMeshPartDefinition(String partName, List<String> path, Matrix4f invertedParentTransform, GeoBone root) implements MeshPartDefinition {
		...
		public Supplier<Matrix4f> getModelPartAnimationProvider() {
			return this.root == null ? () -> null : () -> {
                ...
                return Matrix4fUtils.mulMatrices(parentTransform,
                        new Matrix4f().mul(new Matrix4f().rotation(new Quaternionf().rotationZYX(boneSnapshot.getRotZ(), boneSnapshot.getRotY(), boneSnapshot.getRotX())).transpose().invert())
